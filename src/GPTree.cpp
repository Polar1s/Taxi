...
void read()
{
	...
	// cout<<G.n<<" "<<G.m<<endl;
	// cout<<"correct2"<<endl;
	Additional_Memory=2*G.n*log2(G.n);// (TODO)
	G.init(G.n,G.m);
...
int gptree_search_catch(int S, int T, int bound)
{
	return tree.search_catch(S, T, bound);
}
int gptree_find_path(int S, int T, vector<int> &order)
{
	return tree.find_path(S, T, order);
}
vector<int> gptree_KNN(int S, int K, const vector<int> &T)
{
	return move(tree.KNN(S, K, T));
}
vector<int> gptree_KNN_bound(int S, int K, const vector<int> &T, int bound)
{
	return move(tree.KNN_bound(S, K, T, bound));
}
vector<int> gptree_Range(int S, int R, const vector<int> &T)
{
	return move(tree.Range(S, R, T));
}
